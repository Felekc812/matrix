...
START_TEST(sum_1) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(origin.matrix[i][j], r.matrix[i][j], PRECISION);
    }
  }
  s21_remove_matrix(&m1);
  s21_remove_matrix(&m2);
...
START_TEST(sub_4) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(-(origin.matrix[i][j]), r.matrix[i][j],
                              PRECISION);
    }
  }
  s21_remove_matrix(&m1);
...
START_TEST(mul_num_2) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(0.0, r.matrix[i][j], PRECISION);
    }
  }
  s21_remove_matrix(&m1);
  s21_remove_matrix(&r);
...
START_TEST(mul_num_3) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(m1.matrix[i][j], r.matrix[i][j], PRECISION);
    }
  }
  s21_remove_matrix(&m1);
  s21_remove_matrix(&r);
...
START_TEST(s21_calc_complements_99) {
  matrix_t A, result, res_origin;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(1, 1, &res_origin);
  A.matrix[0][0] = 777;
...
int main(void) {
  ...
  // tcase_add_test(tc1_1, s21_calc_complements_99);
