...
START_TEST(create_11) {
  matrix_t m1;
  int check = s21_create_matrix(4, 2, &m1);
  int check_origin = 0;
  s21_remove_matrix(&m1);
  ck_assert_int_eq(check, check_origin);
}
...
START_TEST(sum_1) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(origin.matrix[i][j], r.matrix[i][j], s21_EPS);
    }
  }
  s21_remove_matrix(&m1);
  s21_remove_matrix(&m2);
...
START_TEST(mul_num_2) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(0.0, r.matrix[i][j], s21_EPS);
    }
  }
  s21_remove_matrix(&m1);
  s21_remove_matrix(&r);
...
START_TEST(mul_num_3) {
  ...
  for (int i = 0; i < r.rows; i++) {
    for (int j = 0; j < r.columns; j++) {
      ck_assert_double_eq_tol(m1.matrix[i][j], r.matrix[i][j], s21_EPS);
    }
  }
  s21_remove_matrix(&m1);
  s21_remove_matrix(&r);
...
int main(void) {
  ...
  tcase_add_test(tc1_1, create_11);
